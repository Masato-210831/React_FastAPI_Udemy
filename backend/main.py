...
def read_user(name:str, password:str, db:Session=Depends(get_db)):
  db_user = crud.get_user_by_name_and_password(db, name=name, password=password)
  if db_user is None:
...
def create_sales(sales:schemas.SalesCreate, db:Session=Depends(get_db)):
  db_sales = crud.get_sales_by_year_and_department(db, year=sales.year, department=sales.department)
  if db_sales:
    raise HTTPException(status_code=404, detail="Sales Info already registered")
...
def read_sales_year(year:int, db:Session=Depends(get_db)):
